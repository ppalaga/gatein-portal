	...
	//				Case ColumnContainer : When dragObject.parent's lastSibling doesn't change
		...
		* Perform following works after dropping :
		...
		findDropableTargets : function(dragBlock) {
			...
			var dragIsContainer = !!jDragObj.closest("#UIContainerList").length;

			var uiRoot = toolPanel.find(".UIPortal");
			if(!uiRoot.length) {
				uiRoot = toolPanel.find(".UIPage");
				console.log("About to check uiPage as root");
			}
			if (((dragIsContainer && uiRoot.hasClass("HasMoveContainersPermissions"))
					|| (!dragIsContainer && uiRoot.hasClass("HasMoveAppsPermissions")))) {
				dropableTargets.push(uiRoot[0]);
				console.log("Adding uiPage as root");
			}
			...
			uiContainers.each(function() {
				...
				if (!jCont.closest(jDragObj).length &&
						...
						((dragIsContainer && jCont.hasClass("HasMoveContainersPermissions")) ||
								(!dragIsContainer && jCont.hasClass("HasMoveAppsPermissions")))
				) {
